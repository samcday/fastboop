    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::InvalidMagic => {
                write!(
                    f,
                    "invalid dev profile magic (expected {DEV_PROFILE_BIN_V0_MAGIC:?})"
                )
            }
            Self::UnsupportedFormatVersion(version) => {
                write!(
                    f,
                    "unsupported dev profile format version {version} (expected {DEV_PROFILE_BIN_FORMAT_V0})"
                )
            }
        }
    }
...
pub fn decode_dev_profile(bytes: &[u8]) -> Result<DeviceProfile, DevProfileCodecError> {
    ...
    if format_version != DEV_PROFILE_BIN_FORMAT_V0 {
        return Err(DevProfileCodecError::UnsupportedFormatVersion(
            format_version,
        ));
    }

    let payload = &bytes[DEV_PROFILE_BIN_V0_HEADER_LEN..];
...
pub fn decode_dev_profile_prefix(
    ...
    let consumed = DEV_PROFILE_BIN_V0_HEADER_LEN
...
pub fn encode_dev_profile(profile: &DeviceProfile) -> Result<Vec<u8>, postcard::Error> {
    ...
    let mut out = Vec::with_capacity(DEV_PROFILE_BIN_V0_HEADER_LEN + payload.len());
    out.extend_from_slice(&DEV_PROFILE_BIN_V0_MAGIC);
    out.extend_from_slice(&DEV_PROFILE_BIN_FORMAT_V0.to_le_bytes());
    out.extend_from_slice(&payload);
    Ok(out)
}

pub fn dev_profile_bin_header_version(bytes: &[u8]) -> Option<u16> {
    if bytes.len() < DEV_PROFILE_BIN_V0_HEADER_LEN {
        return None;
    }
    if bytes[..DEV_PROFILE_BIN_V0_MAGIC.len()] != DEV_PROFILE_BIN_V0_MAGIC {
        return None;
    }
    Some(u16::from_le_bytes([
        bytes[DEV_PROFILE_BIN_V0_MAGIC.len()],
        bytes[DEV_PROFILE_BIN_V0_MAGIC.len() + 1],
    ]))
}
