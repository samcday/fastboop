...
use gibblox_pipeline::{
    LocalArtifactIndex, OpenPipelineOptions, PipelineCachePolicy, PipelineHints, open_pipeline,
    pipeline_identity_string, validate_pipeline_hints,
};
...
#[derive(Default)]
pub struct NativeStage0RootfsFactory;

impl Stage0RootfsFactory for NativeStage0RootfsFactory {
...
impl ArtifactReaderResolver {
    ...
    pub fn with_local_artifacts(paths: &[PathBuf]) -> Result<Self> {
        ...
        let resolver = Self {
            local_artifacts: LocalArtifactIndex::from_paths(paths)?,
            ..Self::default()
        };
        if !paths.is_empty() {
            info!(
    ...
    pub(crate) fn open_artifact_source<'a>(
        ...
        Box::pin(async move {
            let cache_key = pipeline_identity_string(source);
