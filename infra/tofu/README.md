...
- GitHub Actions artifacts are proxied live with CORS + range support:
...
If you want the worker to proxy `/pocketblue/gha/*` and
`/live-pocket-fedora/gha/*`, also set `TF_VAR_artifact_proxy_github_token` to
a GitHub token that can read Actions artifacts from both
`pocketblue/pocketblue` and `samcday/live-pocket-fedora` (for fine-grained
PATs: grant repository `Actions: Read` on both repos).
...
export TF_VAR_artifact_proxy_github_token="<optional token for /<project>/gha/*>"
